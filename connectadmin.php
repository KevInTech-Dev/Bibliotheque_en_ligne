...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $mail = mysqli_real_escape_string($conn, $_POST['profEmail']);
    $mdp = mysqli_real_escape_string($conn, $_POST['profPassword']);
    
    // Vérification des identifiants dans la base de données
    $sql = "SELECT * FROM users_infos WHERE user_email = '$mail' AND user_password = '$mdp' AND id_user_type = 1"; // Vérifiez que l'utilisateur est un administrateur
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        header("Location: table_app_contenant.php?id_user=" . $_SESSION['id_user']); 
        exit;
    } else {
    ...
    <div class="form-container">
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <div class="form-group">
                ...
                <input type="email" id="profEmail" name="profEmail" required>
            </div>
            <div class="form-group">
                <label for="profPassword">Mot de passe</label>
                <input type="password" id="profPassword" name="profPassword" required>
            </div>
        ...
        ?>
        <p>N'avez-vous pas de compte ? <a href="inscritadmin.php">Inscription</a></p>
    </div>
